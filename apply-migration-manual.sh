...
if ! psql -h localhost -U postgres -d ftr_db -c "SELECT 1;" > /dev/null 2>&1; then
  ...
  if docker ps | grep -q postgres; then
    ...
    # Execute migration
    docker exec -i $CONTAINER psql -U postgres -d ftr_db < migration-add-eventId.sql
  ...
  psql -h localhost -U postgres -d ftr_db -f migration-add-eventId.sql
