...
## Как узнать адрес frontend?

### Если frontend запущен локально через Docker:
```bash
# Проверьте docker-compose.yml
cat docker-compose.yml | grep -A 5 "frontend:"
# Обычно это http://localhost:3000/api
```

### Если frontend на удаленном сервере:
1. **Самый простой способ:** Откройте веб-интерфейс в браузере и посмотрите адресную строку
   - Если адрес `http://192.168.1.100:3000` → используйте `http://192.168.1.100:3000/api`
   - Если адрес `https://ftr.lilfil.ru` → используйте `https://ftr.lilfil.ru/api`
2. Спросите у администратора адрес frontend (веб-интерфейса)
3. Проверьте в настройках веб-приложения

### Проверка доступности frontend:
```bash
# Замените на ваш адрес frontend
...
**Важно:** Используйте тот же адрес, что и для веб-интерфейса в браузере, просто добавьте `/api` в конце.
